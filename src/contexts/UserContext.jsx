...
export const UserProvider = ({ children }) => {
  ...
  const logInAsOng = (email, keepLogged) => {
    ...
    setUser(newUser);
    if (keepLogged) localStorage.setItem("user", JSON.stringify(newUser));
  };
  ...
  const logIn = async (data, successCallback, errorCallback, keepLogged) => {
    try {
      ...
      setUser(postData);
      successCallback();
      if (keepLogged) localStorage.setItem("user", JSON.stringify(postData));
    } catch (e) {
      errorCallback(e);
    }
  };
  ...
  return (
    <UserContext.Provider
      value={{ logIn, user, signUp, loggedIn, logOut, logInAsOng }}
    >
      {children}
    </UserContext.Provider>
  );
